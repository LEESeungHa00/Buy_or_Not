...
classifier = load_kr_finbert_model()
...
if data_src == 'CSV 업로드':
    ...
    if not categories:
        ...
        selected_categories = st.sidebar.multiselect("분석할 품목 선택", categories,
...
with tab3:
    ...
    if not raw_news_df.empty:
        news_weekly = raw_news_df.copy()
        news_weekly['날짜'] = pd.to_datetime(news_weekly['날짜'])
        news_weekly = news_weekly.dropna(subset=['Sentiment'])
        news_weekly = news_weekly.set_index('날짜').resample('W-Mon').mean(numeric_only=True)
        news_weekly.index.name = '날짜'
        # 감성 Line/Area & 키워드별 라인 그래프 추가
        fig1 = px.line(news_weekly, y='Sentiment', title="주별 평균 뉴스 감성 점수", labels={'Sentiment': '감성 점수'})
        fig1.add_hline(y=0, line_dash="dash", line_color="red")
        fig2 = px.area(news_weekly, y='Sentiment', title="주별 평균 뉴스 감성 점수 (Area)", labels={'Sentiment': '감성 점수'})
        st.plotly_chart(fig1, use_container_width=True)
        st.plotly_chart(fig2, use_container_width=True)
        if 'Keyword' in raw_news_df.columns and raw_news_df['Keyword'].nunique() > 1:
            news_multikey = raw_news_df.copy()
            news_multikey['날짜'] = pd.to_datetime(news_multikey['날짜'])
            fig3 = px.line(news_multikey, x='날짜', y='Sentiment', color='Keyword', title="키워드별 뉴스 감성 추이")
